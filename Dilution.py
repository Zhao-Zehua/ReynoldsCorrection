...
用于物理化学实验溶解热的积分雷诺校正，可以方便地拟合直线、给出拟合误差、寻找使积分面积相等的点，并求出校正后的温度。
拯救物化实验
——即使它没了
2022.11.22

使用说明
0. 运行python程序，生成图形界面。需安装numpy和scipy库。
1. 点击File导入文件。
2. csv或txt文件格式：两列纯数据，第一列为升序的time(s)，第二列Delta_T(K)；同一行数据间以半角逗号分隔。
3. 调整Start 1 < End 1 < Start 2 < End 2 < Start 3 < End 3至合适位置，推荐单击输入栏后使用键盘↑↓进行调节，报错可以Remake。
4. 点击Integrate积分。
...
P.S. 寻找积分面积相等的点本来可以写二分法的，但不改也不耽误用所以就懒了。以及，写GUI别用Tkinter！
...
csv = None
start1, end1, start2, end2, start3, end3 = None, None, None, None, None, None   # 0 <= start1 < end1 < start2 < end2 < start3 < end3 < len(csv)
len_csv = 6
...
#
def openfile():
    global path, csv, len_csv, start1, end1, start2, end2, start3, end3
    global listbox_csv, label_path, frame_left_3, button_save, button_integrate, button_remake, entry_start1, entry_end1, entry_start2, entry_end2, entry_start3, entry_end3
    # check
    if len_csv < 4:
        len_csv = 4
    # refresh entries
    ...
    # set initial values
    len_csv = len(csv)
    end3.set(str(int(len_csv * 6.00 / 6.00 - 1)))
    start3.set(str(int(len_csv * 5.00 / 6.00 - 1)))
    end2.set(str(int(len_csv * 4.00 / 6.00 - 1)))
    start2.set(str(int(len_csv * 3.00 / 6.00 - 1)))
    end1.set(str(int(len_csv * 2.00 / 6.00 - 1)))
    start1.set(str(int(len_csv * 1.00 / 6.00 - 1)))
    entry_end3.destroy()
    entry_end3 = Spinbox(frame_right_3_middle, textvariable = end3, command = input_end3, from_ = 0, to = len_csv - 1, increment = 1)
    entry_end3.pack(side = LEFT, fill = BOTH, expand = TRUE)
    entry_start3.destroy()
    entry_start3 = Spinbox(frame_right_3_left, textvariable = start3, command = input_start3, from_ = 0, to = len_csv - 1, increment = 1)
    entry_start3.pack(side = LEFT, fill = BOTH, expand = TRUE)
    entry_end2.destroy()
    ...
    entry_start3.config(state = NORMAL)
    entry_end3.config(state = NORMAL)
    # regression
...
def reset_entries():
    ...
    start1.set("0")
    entry_end3 = Spinbox(frame_right_3_middle, textvariable = end3, command = input_end3, from_ = 0, to = len_csv - 1, increment = 1)
    entry_end3.config(state = DISABLED)
    entry_end3.pack(side = LEFT, fill = BOTH, expand = TRUE)
    entry_start3 = Spinbox(frame_right_3_left, textvariable = start3, command = input_start3, from_ = 0, to = len_csv - 1, increment = 1)
    entry_start3.config(state = DISABLED)
    entry_start3.pack(side = LEFT, fill = BOTH, expand = TRUE)
    entry_end2 = Spinbox(frame_right_2_middle, textvariable = end2, command = input_end2, from_ = 0, to = len_csv - 1, increment = 1)
...
def remakefile():
    global csv, len_csv, start1, end1, start2, end2, start3, end3
    global entry_start1, entry_end1, entry_start2, entry_end2, entry_start3, entry_end3
    reset_entries()
    # set initial values
    len_csv = len(csv)
    end3.set(str(int(len_csv * 6.00 / 6.00 - 1)))
...
def calculate_regression():
    global csv, start1, end1, start2, end2, start3, end3
    global text_result
    # check
    ...
    End1 = end1.get()
    Start1 = start1.get()
    if Start1.isdigit() and End1.isdigit() and Start2.isdigit() and End2.isdigit() and Start3.isdigit() and End3.isdigit():
        End3 = int(End3)
        Start3 = int(Start3)
        End2 = int(End2)
        Start2 = int(Start2)
    ...
    k3, b3, stddev_k3, stddev_b3, r_square3 = linear_regression(csv, Start3, End3)[0 : 5]
    # print results
    text_result.config(state = NORMAL)
    text_result.delete('1.0', '10.0')
    text_result_1 = "Linear Fit 1: Delta_T/K = ({:.6} ± {:.3}) t/s + ({:.6} ± {:.3}), r-square = {:.9f}\n".format(k1, stddev_k1, b1, stddev_b1, r_square1)
    text_result_2 = "Linear Fit 2: Delta_T/K = ({:.6} ± {:.3}) t/s + ({:.6} ± {:.3}), r-square = {:.9f}\n".format(k2, stddev_k2, b2, stddev_b2, r_square2)
    text_result_3 = "Linear Fit 3: Delta_T/K = ({:.6} ± {:.3}) t/s + ({:.6} ± {:.3}), r-square = {:.9f}\n".format(k3, stddev_k3, b3, stddev_b3, r_square3)
    text_result.insert('1.0', text_result_1)
    text_result.insert('2.0', text_result_2)
    text_result.insert('3.0', text_result_3)
    text_result.config(state = DISABLED)
    # plot
    dynamic_plot(csv, Start1, End1, Start2, End2, Start3, End3, k1, b1, k2, b2, k3, b3)
    return
def calculate_integrate():
    global path, csv, start1, end1, start2, end2, start3, end3
    global text_result
    # check
    if str(type(csv)) == "<class 'NoneType'>":
        return
    ...
    k3, b3, stddev_k3, stddev_b3, r_square3 = linear_regression(csv, Start3, End3)[0 : 5]
    # Reynolds
    x1, S1_left, S1_right = Reynolds(csv, Start1, End1, Start2, End2)
    x2, S2_left, S2_right = Reynolds(csv, Start2, End2, Start3, End3)
    # print results
    text_result.config(state = NORMAL)
    text_result.delete('1.0', '10.0')
    text_result_1 = "Linear Fit 1: y = ({:.6} ± {:.3})x + ({:.6} ± {:.3}), r-square = {:.6f}\n".format(k1, stddev_k1, b1, stddev_b1, r_square1)
    text_result_2 = "Linear Fit 2: y = ({:.6} ± {:.3})x + ({:.6} ± {:.3}), r-square = {:.6f}\n".format(k2, stddev_k2, b2, stddev_b2, r_square2)
    text_result_3 = "Linear Fit 3: y = ({:.6} ± {:.3})x + ({:.6} ± {:.3}), r-square = {:.6f}\n".format(k3, stddev_k3, b3, stddev_b3, r_square3)
    text_result_4 = "x1 = {:.2f} s  x2 = {:.2f} s\n".format(x1, x2)
    T1_left = k1 * x1 + b1
    T1_right = k2 * x1 + b2
    T2_left = k2 * x2 + b2
    T2_right = k3 * x2 + b3
    text_result_5 = "S1_left = {:.2f} K·s  S1_right = {:.2f} K·s  S2_left = {:.2f} K·s  S2_right = {:.2f} K·s\n".format(S1_left, S1_right, S2_left, S2_right)
    text_result_6 = "T1 = {:.3f} K  T2 = {:.3f} K  T2' = {:.3f} K  T1' = {:.3f} K\n".format(T1_left, T1_right, T2_left, T2_right)
    text_result.insert('1.0', text_result_1)
    text_result.insert('2.0', text_result_2)
    text_result.insert('3.0', text_result_3)
    text_result.insert('4.0', text_result_4)
    text_result.insert('5.0', text_result_5)
    text_result.insert('6.0', text_result_6)
    text_result.config(state = DISABLED)
    # plot
    Reynolds_plot(csv, Start1, End1, Start2, End2, Start3, End3, k1, b1, k2, b2, k3, b3, x1, x2)
    return
...
def input_start1():
    ...
    if Start1 >= 0 and Start1 < End1:
        ...
        # invalid value
        wrong_plot(csv, Start1, End1, Start2, End2, Start3, End3)
        text_result.config(state = NORMAL)
        text_result.delete('1.0', '10.0')
        text_result.insert('1.0', "Required: 0 <= Start 1 < End 1 < Start 2 < End 2 < Start 3 < End 3 <= max(id)\n")
        text_result.insert('2.0', "You can click Remake.\n")
        text_result.config(state = DISABLED)
        button_save.config(state = DISABLED)
        button_integrate.config(state = DISABLED)
        showinfo(title = "Warning", message = "Required: 0 <= Start 1 < End 1 < Start 2 < End 2 < Start 3 < End 3 <= max(id)\nYou can click Remake.\n")
        return False
def input_end1():
    global csv, start1, end1, start2, end2, start3, end3
...
def input_end2():
    ...
    if End2 <= Start2:
        ...
        return False
    """
    if End2 > Start2 and End2 < Start3:
        calculate_regression()
        button_save.config(state = NORMAL)
        button_integrate.config(state = NORMAL)
        return True
...
def input_start3():
    ...
    if Start2 <= End1:
        ...
        return False
    """
    if Start3 > End2 and Start3 < End3:
        calculate_regression()
        button_save.config(state = NORMAL)
        button_integrate.config(state = NORMAL)
        return True
        ...
        return False
def input_end3():
    global csv, start1, end1, start2, end2, start3, end3, len_csv
    global text_result, entry_end2, frame_right_2_middle, button_save, button_integrate
    # check
    End3 = end3.get()
    Start3 = start3.get()
    ...
    if End2 <= Start2:
        ...
        return False
    """
    if End3 > Start3 and End3 < len_csv:
        calculate_regression()
        button_save.config(state = NORMAL)
        button_integrate.config(state = NORMAL)
        return True
...
def wrong_plot(csv, Start1, End1, Start2, End2, Start3, End3, dx = 0.05):
    ...
    f.scatter(csv[Start3, 0], csv[Start3, 1], s = 15, color = 'mediumpurple', label = "linear fit 3 endpoints")
    f.scatter(csv[End3, 0], csv[End3, 1], s = 15, color = 'mediumpurple')
    # smooth curve
    ...
    f.set_xlabel("$t$ (s)")
    f.set_ylabel("$\Delta T$ (K)")
    f.text(np.max(csv, 0)[0] * 0.2 + np.min(csv, 0)[0] * 0.8, np.max(csv, 0)[1] * 0.5 + np.min(csv, 0)[1] * 0.5, s = "Required: 0 <= Start 1 < End 1 < Start 2 < End 2 < Start 3 < End 3 <= max(id)\nYou can click Remake.\n", color = 'red')     # remove first # to display warning
    f.legend()
    canvas_Reynolds.draw()
    return
def dynamic_plot(csv, Start1, End1, Start2, End2, Start3, End3, k1, b1, k2, b2, k3, b3, dx = 0.05):
    global P, f, canvas_Reynolds
    # clear figure
    f.clear()
    # scatter
    ...
    f.plot(x_curve, y3, ls = "--", linewidth = 1, color = 'mediumpurple', label = "linear fit 3")
    # other plot settings
    f.set_xlabel("$t$ (s)")
    f.set_ylabel("$\Delta T$ (K)")
    f.legend()
    canvas_Reynolds.draw()
    return
def Reynolds_plot(csv, Start1, End1, Start2, End2, Start3, End3, k1, b1, k2, b2, k3, b3, x1, x2, dx = 0.05):
    global P, f, canvas_Reynolds
    # clear figure
    f.clear()
    # scatter
    ...
    T1_right = k2 * x1 + b2
    if T1_left > T1_right:
        T1_left, T1_right = T1_right, T1_left
    dT1 = T1_right - T1_left
    y4 = np.arange(T1_left - dT1 * 0.1, T1_right + dT1 * 0.1, dx)
    x4 = np.full(len(y4), x1)
    f.plot(x4, y4, ls = "--", color = 'red', linewidth = 1, label = "Reynolds auxiliary line")
    T2_left = k2 * x2 + b2
    T2_right = k3 * x2 + b3
    if T2_left > T2_right:
        T2_left, T2_right = T2_right, T2_left
    dT2 = T2_right - T2_left
    y5 = np.arange(T2_left - dT2 * 0.1, T2_right + dT2 * 0.1, dx)
    x5 = np.full(len(y5), x2)
    f.plot(x5, y5, ls = "--", color = 'red', linewidth = 1)
    # integrate area
    smooth = interpolate.interp1d(csv[:, 0], csv[:, 1], kind = 'cubic')
    x_area1_left = np.arange(csv[End1, 0], x1, dx)
    x_area1_right = np.arange(x1, csv[Start2, 0], dx)
    y_area1_left_linear = k1 * x_area1_left + b1
    y_area1_right_linear = k2 * x_area1_right + b2
    y_area1_left_smooth = smooth(x_area1_left)
    y_area1_right_smooth = smooth(x_area1_right)
    f.fill_between(x_area1_left, y_area1_left_linear, y_area1_left_smooth, color = 'grey', alpha = 0.2)
    f.fill_between(x_area1_right, y_area1_right_linear, y_area1_right_smooth, color = 'grey', alpha = 0.2)
    x_area2_left = np.arange(csv[End2, 0], x2, dx)
    x_area2_right = np.arange(x2, csv[Start3, 0], dx)
    y_area2_left_linear = k2 * x_area2_left + b2
    y_area2_right_linear = k3 * x_area2_right + b3
    y_area2_left_smooth = smooth(x_area2_left)
    y_area2_right_smooth = smooth(x_area2_right)
    f.fill_between(x_area2_left, y_area2_left_linear, y_area2_left_smooth, color = 'grey', alpha = 0.2)
    f.fill_between(x_area2_right, y_area2_right_linear, y_area2_right_smooth, color = 'grey', alpha = 0.2)
    # other plot settings
    f.set_xlabel("$t$ (s)")
    f.set_ylabel("$\Delta T$ (K)")
    f.legend()
...
def main():
    global path, csv, start1, end1, start2, end2, start3, end3, len_csv
    ...
    start3 = StringVar(master = root)
    end3 = StringVar(master = root)
    end3.set("5")
    start3.set("4")
    end2.set("3")
    start2.set("2")
    ...
    frame_right_3 = Frame(frame_right, borderwidth = 2)
    frame_right_3.pack(fill = BOTH)
    # root/frame_right/frame_right_3/frame_right_3_left
    global frame_right_3_left
    frame_right_3_left = Frame(frame_right_3, padx = 30, pady = 10)
    frame_right_3_left.pack(side = LEFT, fill = BOTH, expand = TRUE)
    # root/frame_right/frame_right_3/frame_right_3_left/label_start3
    label_start3 = Label(frame_right_3_left, text = "Start 3", padx = 10)
    label_start3.pack(side = LEFT, fill = BOTH)
    # root/frame_right/frame_right_3/frame_right_3_left/entry_start3
    global entry_start3
    entry_start3 = Spinbox(frame_right_3_left, textvariable = start3, command = input_start3, from_ = 0, to = len_csv - 1, increment = 1)
    entry_start3.config(state = DISABLED)
    entry_start3.pack(side = LEFT, fill = BOTH, expand = TRUE)
    # root/frame_right/frame_right_3/frame_right_3_middle
    global frame_right_3_middle
    frame_right_3_middle = Frame(frame_right_3, padx = 30, pady = 10)
    frame_right_3_middle.pack(side = LEFT, fill = BOTH, expand = TRUE)
    # root/frame_right/frame_right_3/frame_right_3_middle/label_end3
    label_end3 = Label(frame_right_3_middle, text = "End 3", padx = 10)
    label_end3.pack(side = LEFT, fill = BOTH)
    # root/frame_right/frame_right_3/frame_right_3_middle/entry_end3
    global entry_end3
    entry_end3 = Spinbox(frame_right_3_middle, textvariable = end3, command = input_end3, from_ = 0, to = len_csv - 1, increment = 1)
    entry_end3.config(state = DISABLED)
    entry_end3.pack(side = LEFT, fill = BOTH, expand = TRUE)
    # root/frame_right/frame_right_3/frame_right_3_right
    frame_right_3_right = Frame(frame_right_3)
    frame_right_3_right.pack(side = LEFT, fill = BOTH)
    # root/frame_right/frame_right_3/frame_right_3_right/button_save
    global button_save
    button_save = Button(frame_right_3_right, text = 'Save (*.png)', command = savefile, height = 2, width = 20)
    button_save.config(state = DISABLED)
    button_save.pack()
    # root/frame_right/frame_right_4
    frame_right_4 = Frame(frame_right, borderwidth = 5, relief = SUNKEN)
    frame_right_4.pack(fill = BOTH)
    # root/frame_right/frame_right_4/text_result
    global text_result
    text_result = Text(frame_right_4, height = 7, wrap = CHAR, padx = 8, pady = 8)
    ...
    text_result.insert('4.0', "3. 调整Start 1 < End 1 < Start 2 < End 2 < Start 3 < End 3至合适位置，推荐单击输入栏后使用键盘↑↓进行调节，报错可以Remake。\n")
    ...
    # root/frame_right/frame_right_5
    frame_right_5 = Frame(frame_right, borderwidth = 5, relief = SUNKEN)
    frame_right_5.pack(fill = BOTH, expand = TRUE)
    # root/frame_right/frame_right_5/canvas_Reynolds
    global P, canvas_Reynolds
    canvas_Reynolds = FigureCanvasTkAgg(P, master = frame_right_5)
    canvas_Reynolds.draw()
    canvas_Reynolds.get_tk_widget().pack(fill = BOTH, expand = TRUE)
    toolbar = NavigationToolbar2Tk(canvas_Reynolds, frame_right_5)
    toolbar.update()
